...
export function Home(){
    ...
    return(
        <>
            <div className="d-flex justify-content-center flex-wrap" style={{width:90+"%",margin:"auto"}}>
               {
                   collection.map(item=>
                    <div className="card mt-4 ms-4" key={item.show.id}>
                        <div className="card-header fw-bold text-center">{item.show.name}</div>
                    <div className="card-body">
                        <Link to={`details/${item.show.id}`}><img src={item.show.image.medium} alt="pic"/></Link>
                    </div>
                    <div className="card-footer d-flex justify-content-between">
                        <span className="bg-success text-white p-1 border border-2 rounded"><i className="bi bi-star-fill"></i>&nbsp;{item.show.rating.average}</span>
                        <span className="fw-semibold">IMDB : {item.show.externals.imdb}</span>
                    </div>
                    </div>
                    )
                }
