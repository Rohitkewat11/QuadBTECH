...
export function Details() {
  const collection = [
    ...
    },
    {
      ...
      show: {
        ...
        summary:
          "The new kids of comedy bring the funny as <b>All That, America's #1 kid's sketch comedy show, returns with a new cast and a few familiar faces. From Executive Producers Kenan Thompson and Kel Mitchell.",
        updated: 1702838271,
        _links: {
    ...
    },
    {
      ...
      show: {
        ...
        summary:
          "All Stars follows a former cheerleader from Daytona Beach who cons her way across the pond to teach cheerleading at a school in coastal England and has to show a ragtag group of students – and herself – how to be All Stars.",
        updated: 1704306696,
    ...
    },
    {
      ...
      show: {
        ...
        _links: {
          ...
          previousepisode: { href: "https://api.tvmaze.com/episodes/2686287" },
        },
      },
    },
    {
  ...
  let id = useParams();
  const newID = Object.values(id);


  useEffect(() => {
    var temp = collection.find((val) => val.show.id === parseInt(newID));
    setData(temp.show);
  },[]);
  return (
    <>
      <div
        className="row mt-5 border border-2 rounded-3 p-3"
        style={{ width: 80 + "%", margin: "auto" }}
      >
        <div className="col-4">
          <img src={data.image.original} alt="pic" height={400} width={300} />
        </div>

        <div className="col-6">
          <div>
          <h3>Show Name:- {data.name}</h3>
          <div>
          <span className="bg-success text-white p-1 border border-2 rounded me-2"><i className="bi bi-star-fill"></i>&nbsp;{data.rating.average}</span>
            <span className="fw-semibold me-2">Language:&nbsp;{data.language},</span>
            <span className="fw-semibold me-2">Type:&nbsp;{data.type}</span>
            <span className="fw-semibold">Premiered:{data.premiered}</span>
          <p className="fw-semibold mt-3">{data.summary}</p>
          </div>
          </div>
        <div className="text-end">
            <button className="btn btn-success me-3" type="button" >BooK</button>
            <Link to="/"><button className="btn btn-primary" type="button">Back to Shows</button></Link>
        </div>
        </div>
      </div>

      {/* <div className="border border-3 rounded-3 w-25 p-2" style={style}>
        <form action="">
            ...
            <input className="form-control" type="text" value={data.schedule.time}/>
            <label htmlFor="" className="form-label">Day</label>
            <input className="form-control" type="text" value={data.schedule.day}/>
